...
var wrap = function(wrapper, str) {
  var lw = "";
  var rw = "";
  for (index = 0; index < wrapper.length; ++index) {
    char = wrapper[index];
    if (index == Math.floor(wrapper.length/2)) {
      if ((wrapper.length % 2) == 1) {
        lw += char;
        rw += char;
      } else {
        rw += char;
      }
    } else if (index > wrapper.length/2) {
      rw += char;
    } else {
      lw += char;
    }
...
var throwErr = function(message, logMessage=null) {
  if (logMessage != null) {
    logging(logMessage);
  }
